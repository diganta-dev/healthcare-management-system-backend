...
const updateSchedule = async (
	...
	if (
		...
		throw new AppError(
			httpStatus.BAD_REQUEST,
			"Cannot update a published schedule and appointments have been booked",
		);
	}
	if (
		payload.startDateTime &&
		payload.endDateTime &&
		isAfter(payload.startDateTime, payload.endDateTime)
	) {
		throw new AppError(
			httpStatus.BAD_REQUEST,
			"Start date time must be before end date time",
	...
	if (
		payload.startDateTime &&
		payload.endDateTime &&
		isSameDay(payload.startDateTime, payload.endDateTime) === false
	) {
		throw new AppError(
			httpStatus.BAD_REQUEST,
			"Start date time and end date time must be on the same day",
...
const deleteSchedule = async (scheduleId: string, doctorId: string) => {
	...
	if (
		...
		throw new AppError(
			httpStatus.BAD_REQUEST,
			"Cannot delete a published schedule and appointments have been booked",
		);
	}
	if (schedule.isDeleted) {
		throw new AppError(httpStatus.NOT_FOUND, "Schedule not found");
